@extends('layouts.admin.app', ['body_class' => '', 'title' => 'Edit Laboratory'])
@section('content')
    <div class="container-fluid">
        ...
        <form action="{{ route('admin.laboratory.update', $laboratory) }}" enctype="multipart/form-data" method="POST">
            @csrf
            @method('PATCH')
            <div class="row">
                <div class="col-12 col-md-12 col-xl-8 col-left">
                    <div class="card mb-4">
                        <div class="card-body">
                            <x-input name="name" text="Name" required :model="$laboratory" />
                            @livewire('seo-url', ['model' => 'App\\Models\\Pages\\Laboratory', 'model_id' => $laboratory->id])
                            <div class="form-group">
                                <label for="name">Content</label>
                                <textarea class="form-control" name="content" id="engEditor">{!! $laboratory->content !!}</textarea>
                            </div>
                            <x-input name="main_video" text="Main Youtube Video" type="url" :model="$laboratory" />
                            ...
                            <div class="dynamic-wrap">
                                ...
                                @if ($laboratory->videos)
                                    @foreach (json_decode($laboratory->videos) as $video)
                    ...
                    <div class="card mb-4">
                        <div class="card-body">
                            ...
                            <select name="status" class="form-control select2-single mb-3">
                                <option {{ old('status', $laboratory->status) == '1' ? 'selected' : '' }} value="1">
                                    Enabled
                                </option>
                                <option {{ old('status', $laboratory->status) == '0' ? 'selected' : '' }} value="0">
                                    Disabled
                                </option>
                            </select>
                            <hr />
                            ...
                            <div class="image-container">
                                <img id="featuredPreview" src="{{ $laboratory->featuredImage() }}"
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" type="text" name="image_alt"
                                    value="{{ old('image_alt', $laboratory->image_alt) }}">
                            </div>

                            <x-input name="sort_order" text="Sort Order" :model="$laboratory" />
    ...
    <form action="{{ route('admin.laboratory.destroy', $laboratory) }}" method="POST" id="deleteForm">
        @csrf
        @method('DELETE')
    </form>
@endsection
