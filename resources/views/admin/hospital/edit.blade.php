@extends('layouts.admin.app', ['body_class' => '', 'title' => 'Edit Hospital'])
@section('content')
    <div class="container-fluid">
            ...
            <div class="col-12">
                ...
                <nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
                    <ol class="breadcrumb pt-0">
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{ route('admin.hospital.index') }}">Hospitals</a>
                        </li>
        ...
        <form action="{{ route('admin.hospital.update', $hospital) }}" enctype="multipart/form-data" method="POST">
            @csrf
            @method('PATCH')
            <div class="row">
                <div class="col-12 col-md-12 col-xl-8 col-left">
                    <div class="card mb-4">
                        <div class="card-body">
                            <x-input name="name" text="Name" required :model="$hospital" />
                            @livewire('seo-url', ['model' => 'App\\Models\\Pages\\Hospital', 'model_id' => $hospital->id])
                            <div class="form-group">
                                <label for="name">Content</label>
                                <textarea class="form-control" name="content" id="engEditor">{!! $hospital->content !!}</textarea>
                            </div>
                            <x-input name="main_video" text="Main Youtube Video" type="url" :model="$hospital" />
                            ...
                            <div class="dynamic-wrap">
                                ...
                                @if ($hospital->videos)
                                    @foreach (json_decode($hospital->videos) as $video)
                    ...
                    <div class="card mb-4">
                        <div class="card-body">
                            ...
                            <select name="status" class="form-control select2-single mb-3">
                                <option {{ old('status', $hospital->status) == '1' ? 'selected' : '' }} value="1">
                                    Enabled
                                </option>
                                <option {{ old('status', $hospital->status) == '0' ? 'selected' : '' }} value="0">
                                    Disabled
                                </option>
                            </select>
                            <hr />
                            ...
                            <div class="image-container">
                                <img id="featuredPreview" src="{{ $hospital->featuredImage() }}"
                            ...
                            <x-input name="sort_order" text="Sort Order" :model="$hospital" />
    ...
    <form action="{{ route('admin.hospital.destroy', $hospital) }}" method="POST" id="deleteForm">
        @csrf
        @method('DELETE')
    </form>
@endsection
