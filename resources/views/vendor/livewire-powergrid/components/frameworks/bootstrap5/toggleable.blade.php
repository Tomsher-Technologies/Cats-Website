<div x-data="pgToggleable({
    ...
    @if ($column->toggleable['enabled'])
        {{-- <div class="form-check form-switch">
            <label>
                <input x-on:click="save()" class="form-check-input" :checked="toggle === 1" type="checkbox">
            </label>
        </div> --}}

        <div class="custom-switch custom-switch-secondary mb-2 form-check form-switch">
            <input x-on:click="save()" class="custom-switch-input form-check-input" id="switch{{ $row->{$primaryKey} }}" type="checkbox" {{ $row->{$column->field} == 1 ? "checked" : '' }}>
            <label class="custom-switch-btn" for="switch{{ $row->{$primaryKey} }}"></label>
        </div>
    @else
